...
- `src/pages/index`：跨端工作台 UI，包含行业工作域、分类元件库、商业化面板、学习路径、训练挑战、知识验证、连接画布、属性检查器、训练评分、安全诊断和仿真结果。
...
- 学习卡、训练卡、题库 Track、选项和仿真效果条改为横向可滚动，避免卡片过窄导致文字挤压。
...
7. 在 `src/pages/index/index.scss` 中补充 `visual-*` 示意图样式。元件库、画布和属性面板都会复用该图形。
