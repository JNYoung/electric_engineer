...
- Commercial catalogs split engineering-control and renovation-control domains, keep category counts aligned, gate premium components by plan tier, build account/billing access snapshots, and expose the auth/billing API contract.
...
- The H5 e2e flow switches between engineering industrial control and renovation control tabs, confirms domain-specific components, verifies locked-feature summaries, and exercises backend-backed sign-in/payment entry points.
